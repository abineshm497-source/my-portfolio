...
// Auto-sync profile photo & PDF resume from uploaded user media
try {
  const newPhotoSrc = 'C:/Users/USER/.gemini/antigravity-ide/brain/287bbd58-f7b1-4998-aa41-1a0a7b5d550b/media__1788677565031.jpg';
  if (fs.existsSync(newPhotoSrc)) {
    fs.copyFileSync(newPhotoSrc, path.join(__dirname, 'profile.jpg'));
    const parentDir = path.dirname(__dirname);
    fs.copyFileSync(newPhotoSrc, path.join(parentDir, 'profile.jpg'));
  }

  const pdfSrc = 'C:/Users/USER/.gemini/antigravity-ide/brain/287bbd58-f7b1-4998-aa41-1a0a7b5d550b/media__1788674145152.pdf';
  if (fs.existsSync(pdfSrc)) {
    fs.copyFileSync(pdfSrc, path.join(__dirname, 'Abinesh_M_Resume.pdf'));
    const parentDir = path.dirname(__dirname);
    fs.copyFileSync(pdfSrc, path.join(parentDir, 'Abinesh_M_Resume.pdf'));
    console.log('✅ Synchronized profile photo and PDF resume to workspace.');
  }
} catch (syncErr) {
  console.log('Media sync note:', syncErr.message);
}
